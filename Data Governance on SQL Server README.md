...
2: Select the Database on which Users must be created. Create the Users on the 'ClinicalTrials Database to map the same login credentials from Step 1. Apply Role-Based Permissions (RBAC) on ClinicalTrials Database only. 
...
5. Total 5 tables and 1 view would be visible from the login user Business Analyst on Power BI.
